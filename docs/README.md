...
|   text   |       提示文字       |        str        | '请填写' |
| command  | 点击按钮时执行的内容 | function / lambda |   None   |
...
FlatButton是一个继承自tkinter.Label伪按钮控件，旨在为Button提供更加现代的外观。FlatButton使用扁平化直角设计，颜色可以自定义。

**尚无可用图片，若图片就绪，将会及时加在此处。**
...
|   parent   |       父级       |       tk控件       |            |
|    text    |     按钮文本     |        str         |    None    |
|   image    |     按钮图像     | ImageTk.PhotoImage |    None    |
|     bg     |      背景色      |        str         | "#0078dc"  |
|     fg     |      前景色      |        str         | "#ffffff"  |
| floatingbg | 鼠标悬浮时背景色 |        str         |  "darker"  |
| floatingfg | 鼠标悬浮时前景色 |        str         | "nochange" |
...
用于处理颜色参数中的"lighter"、"darker"，开发者无需自行调用。
...
点击按钮时执行，会直接调用command传入的内容。本函数用于防止`None object is not callable`
